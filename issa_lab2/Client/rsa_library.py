...
def encrypt(public_key, hex_number):
    ...
    result = integer_number ** public_key[0] % public_key[1]
    return result


############################### EXERCISE 2 ###############################
def decrypt(private_key, encrypted_msg):
    result = encrypted_msg ** private_key[0] % private_key[1]
    return result


############################### EXERCISE 3 ###############################
def low_check(hex_nr):
    binary_number = bin(int(hex_nr, 16))[2:]
    low_bits = binary_number[8:]
    binary_01 = bin(int("01", 16))[2:]
    return int(low_bits) == int(binary_01)
...
def number_check(hex_nr):
    if low_check(hex_nr):
        binary_number = bin(int(hex_nr, 16))[2:]
        low_bits = int(binary_number[8:], 2)
        high_bits = int(binary_number[0:8], 2)
        return low_bits | high_bits == int("FF", 16)
    return False
